# This is a simple program that takes input from the command line and processes it accordingly
# October 23, 2017 

# Import allow us to use functions from pre-written packages
# The format is: import package-name
import sys

# We can pass information to a program when it runs, this allows us to pass imformation from other scripts
# We check the number of command line arguments with the variable:
num_args=len(sys.argv)

# This declares an empty list that we can add to later
# A list is different from an array in the sense it can grow; array size is fixed
arg_storage=[]

# Now that we have declared some variables lets make a function to check what the script was passed
def argHandler():
    ...
    # This section executes if the first statement fails
        ...
        # We already stored the number of argumemts so we have our bound for looping
...
# Once we have built our list we print the contents
# For that we will make another function to print it
